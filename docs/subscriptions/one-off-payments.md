...
* [Flow 3](#flow-3---one-off-with-auto-reserve): Merchants can send One-Off payment, which MobilePay will attempt to automatically reserve, without user’s confirmation
...
* The link *rel = mobile-pay* hyperlink reference must be used to redirect the user automatically using an HTTP response 302 or 303. Once the user is redirected, the MobilePay app will be opened to confirm the **One-off Payment**. This applies only if `auto_reserve` field is omitted or set to **false**.
...
|Expired   |_1. One-off payment was neither accepted, nor rejected by user. 2. User didn't any action after automatic reservation failed._| 1 day after you requested one-off payment |Expired|Expired by system.| 50008 |
