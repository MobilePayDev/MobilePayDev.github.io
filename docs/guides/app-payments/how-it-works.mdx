...
1. When a user wants to pay with MobilePay in your application, request your backend services to [create payment](/docs/payments-refunds/create-payments#create-payment) using [Payments API](/docs/payments-refunds/create-payments).
2. Re-direct user to MobilePay app using deep link.
3. User confirms payment in the MobilePay app.
4. Your backend services receives notification `payment.reserved` and proceeds with [capturing payment](/docs/payments-refunds/create-payments#capture-payment).
5. Meanwhile user returns to your application (using re-direct or switching apps).
6. Invoke backend services to check payment status. It is `captured` and you should proceed with the success screen.
...
6. Backend services haven't received a notification and should invoke [Get Single Payments Details](/docs/payments-refunds/create-payments#get-single-payment-details) to retrieve the latest payment state and decide what to do next.
