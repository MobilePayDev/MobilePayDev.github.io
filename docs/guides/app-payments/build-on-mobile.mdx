...
Activity Result or `startActivityForResult` APIs use standard android functions and will launch MobilePay in your app process. This is the preferred way when integrating into native android app.
...
```kotlin title="Kotlin using Activity Result APIs (Requires AndroidX Activity 1.2.0 or Fragment 1.3.0) "
...
// Use the link received from backend to launch MobilePay.
...
MobilePay can return these result codes:
  ...
  - <b>FAILED_RESULT_CODE</b> (1) - user could not complete the payment due to invalid state of payment or his MobilePay account.
  ...
  (E.g. User tries to do a payment, but it does not succeed due to network issues. Then he decides to reject the payment and again he receives a network error. 
  At this point we cannot leave him blocked in MobilePay app, so we finish with this result code.) 
...
Deep link approach is used when MobilePay app is launched with `startActivity` function. When using this navigation method the MobilePay app will run in separate process and invoke your app with a deep link after payment flow is finished.
